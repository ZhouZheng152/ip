...
public class Vega {
    public static void main(String[] args) {
        Storage storage = new Storage("data/vega.txt");
        ...
        try (Scanner scanner = new Scanner(System.in)) {
            while (true) {
                ...
                try {
                    handleCommand(tasks, command, storage);
                } catch (VegaException e) {
    ...
    private static void handleCommand(ArrayList<Task> tasks, String command, Storage storage) throws VegaException {
        if (command.equals("list")) {
            printList(tasks);
        } else if (command.equals("mark") || command.startsWith("mark ")) {
            updateTaskStatus(tasks, command, true, storage);
        } else if (command.equals("unmark") || command.startsWith("unmark ")) {
            updateTaskStatus(tasks, command, false, storage);
    ...
    private static void updateTaskStatus(ArrayList<Task> tasks, String command, boolean shouldMark, Storage storage)
    ...
    private static void deleteTask(ArrayList<Task> tasks, String command, Storage storage) throws VegaException {
    ...
    private static void addTask(ArrayList<Task> tasks, Task task, Storage storage) throws VegaException {
        tasks.add(task);
        storage.saveTasks(tasks);
        printLine();
    ...
    private static void addEvent(ArrayList<Task> tasks, String command, Storage storage) throws VegaException {
        ...
        addTask(tasks, new Event(fromParts[0].trim(), toParts[0].trim(), toParts[1].trim()), storage);
    }
